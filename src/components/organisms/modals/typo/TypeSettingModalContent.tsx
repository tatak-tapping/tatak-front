...
const TypeSettingModalContent = ({ onClickCloseModal }: TypeSettingModalContentProps) => {
  ...
  const dataArray = categoryWithTopic
    .map((item_x, index_x) => {
      return item_x.topics.map((item_y, index_y) => item_y.topicCode.toString());
    })
    .reduce(function (pre, cur) {
      return pre.concat(cur);
    });

  const initialState: toggleCheckProps = dataArray.reduce(
    (o, key: string) => ({
      ...o,
      [key]: false,
    }),
    ...
    <Flex flexDirection="column" ml="20px" mr="20px" width="696px">
      ...
      <Box mt="20px">
        <Flex flexDirection="inherit">
          ...
          <StyledClickText>
            <span onClick={handleToggleAllCheck}>{checkedAll ? '전체 해제' : '전체 선택'}</span>
          </StyledClickText>
        </Flex>
