...
export const getArticle = (data?: getTypoArticleParams): Promise<{ data: ITypo }> => {
  if (data) {
    return instance.get(
      `/articles/main?languages=${data.languages}&topicCodes=${data.topicCodes}&lengths=${data.lengths}`
    );
  }
  return instance.get(`/articles/main`);
};

export const getCategories = (): Promise<{ data: ICategory[] }> => instance.get(`/categories`);
